...
def construct_features_df(list_of_ts: Dict[int, Dict[str, List[float]]], list_of_feature_fns: List[callable]) -> Tuple[
    pd.DataFrame, pd.Series]:
    """
    Constructs a DataFrame of features from a list of time series.

    Args:
        list_of_ts: A dictionary of time series data, where the keys are the time series IDs and the values are dictionaries containing the time series data and the label.
    ...
    for ts_id, ts_data in tqdm(list_of_ts.items()):
        ...
        y.append(ts_data['label'])

    return pd.DataFrame(features), pd.Series(y)


def process_ts(list_of_feature_fns: List[callable], ts_data: Tuple[int, Dict[str, List[float]]]) -> Tuple[
    List[float], int]:
    """
    Processes a single time series to extract features and label.
    ...
    Returns:
        A tuple containing the extracted features and the label.
    """
    ts_id, data = ts_data
...
def construct_features_df_parallel(list_of_ts: Dict[int, Dict[str, List[float]]],
                                   list_of_feature_fns: List[callable]) -> Tuple[pd.DataFrame, pd.Series]:
    """
    Constructs a DataFrame of features from a list of time series in parallel.
